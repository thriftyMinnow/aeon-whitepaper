...
A single block of AEON transactions is added to the public blockchain once every 4 minutes, on average.  This timing must be ensured by constantly adjusting the \textit{difficulty} of the Proof Of Work calculation.  Otherwise the addition of more miners to the network would result in calculating the correct PoW faster than the desired 4 minutes.\\
\\
...
\\
The following graph shows the number of coins rewarded for each block over time.

\begin{center}
	\includegraphics[width=5.5in]{aeonEmissionCurve}
\end{center}
...
\begin{center}
	\includegraphics[width=5.5in]{aeonSupplyCurve}
\end{center}
...
% 
%Is there a base transaction fee?  Fee equation?
%Is the reward penalty the same as Monero?  
